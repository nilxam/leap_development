...
class FindSLE(object):
    ...
    def crawl(self):
        ...
        factory_pkglist = self.get_source_packages(FACTORY)
        bp_pkglist = self.get_source_packages(BACKPORTS)
        bp_fails = self.get_prj_results(BACKPORTS, 'x86_64', code='failed')
        sle_ones = []
        deletes = []
        sle_fails = []

        for pkg in bp_pkglist:
            if pkg not in factory_pkglist:
        ...
        for pkg in bp_fails:
            if pkg in sle_pkglist:
                sle_fails.append(pkg)

        print("\nSLE build failed in Backports:")
        for pkg in sle_fails:
            print(pkg)
